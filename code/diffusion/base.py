...
from MIE.model.unet_autoenc import AutoencReturn
from MIE.config_base import BaseConfig
import enum
import math

import numpy as np
import torch as th
from MIE.model import *
from MIE.model.nn import mean_flat
from typing import NamedTuple, Tuple
from MIE.choices import *
...
class GaussianDiffusionBeatGans:
    ...
    def __init__(self, conf: GaussianDiffusionBeatGansConfig):
        ...
        # print(type(self.model_var_type))
        # # 检查实际值
        # print(repr(self.model_var_type))  # 打印详细的字符串表示
        #
        # # 如果可能，检查赋值过程
        # print(self.model_var_type.__class__)  # 确认类型
        #
        # print(id(self.model_var_type))  # ID of the current instance
        # print(id(ModelVarType.fixed_large))  # ID of the enum instance
        #
        # print(self.model_var_type == ModelVarType.fixed_large)
        # print(self.model_var_type == ModelVarType.fixed_small)
    ...
    def training_losses(self,
        ...
        #         LossType.l1,
        # ]:
        # print(self.loss_type.name)
        # print(self.loss_type.name == 'mse')
        if self.loss_type.name == 'mse' or self.loss_type.name == 'l1':
    ...
    def p_mean_variance(self,
        ...
        # print(self.model_var_type.name)

        if self.model_var_type.name ==  'fixed_large' or self.model_var_type.name ==  'fixed_small':
            # print(type(self.model_var_type))
            # # 检查实际值
            # print(repr(self.model_var_type))  # 打印详细的字符串表示
            #
        ...
        # print(self.model_mean_type.name)

        if self.model_mean_type.name == 'eps':
            if self.model_mean_type.name == 'eps':
